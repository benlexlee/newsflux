...
export default function ChessGame() {
  ...
  useEffect(() => {
    if (turn === 'black' && !gameOver && !aiThinking && gameStarted) {
      const timer = setTimeout(() => makeAIMove(), 200);
    ...
    <div ref={elementRef} className="w-full h-full min-h-[500px] bg-gray-900/50 rounded-xl p-4">
      ...
      {showInstructions && (
        <div className="bg-gray-800 p-4 rounded-lg mb-4 text-sm">
          ...
          <ul className="list-disc list-inside"><li>Click a white piece, then click a square to move it.</li><li>White moves first. Computer plays black.</li><li>Timer starts on your first move.</li><li>Capturing gives a different sound.</li></ul>
        </div>
      )}
      {showLeaderboard && (
