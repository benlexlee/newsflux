    ...
    <div ref={elementRef} className="w-full h-full min-h-[500px] bg-gray-900/50 rounded-xl p-4">
      ...
      {showInstructions && (
        <div className="bg-gray-800 p-4 rounded-lg mb-4 text-sm">
          ...
          <ul className="list-disc list-inside"><li>Fill empty cells with numbers 1-9.</li><li>Each row, column, and 3x3 box must contain all numbers 1-9 without repeats.</li><li>Timer starts on first entry.</li></ul>
        </div>
      )}
      {showLeaderboard && (
        <div className="bg-gray-800 p-4 rounded-lg mb-4 shadow">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {leaderboard.length===0 && <tr><td colSpan="4" className="text-center text-gray-500">No times yet. Solve a puzzle!</td></tr>}
            </tbody>
          </table>
              ...
              <tr key={i}>
                {row.map((cell,j)=>(
                  <td key={j} className={`border border-gray-500 ${(i+1)%3===0 && i!==8 ? 'border-b-4' : ''} ${(j+1)%3===0 && j!==8 ? 'border-r-4' : ''}`}>
                    <input type="number" min="1" max="9" value={cell||''} onChange={(e)=>handleCellChange(i,j,e.target.value)} className="w-12 h-12 text-center text-xl focus:outline-none focus:bg-blue-900 bg-gray-800 text-white" disabled={cell!==null || justCompleted} />
                  </td>
                ))}
              </tr>
            ))}
